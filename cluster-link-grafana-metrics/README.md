...
```shell
    docker compose exec kafka-1 kafka-topics --bootstrap-server kafka-1:19092 --topic product --create --partitions 1 --replication-factor 1
    docker compose exec kafka-1 kafka-topics --bootstrap-server kafka-1:19092 --topic other-topic --create --partitions 1 --replication-factor 1
```

### Open a consumer on main cluster

```shell
    docker compose exec kafka-dr-1 \
        kafka-avro-console-consumer --bootstrap-server kafka-1:19092 \
        --property schema.registry.url=http://kafka-dr-1:8085 \
        --group test-group \
        --from-beginning \
        --topic product
```
...
```shell
   docker compose exec kafka-dr-1 kafka-avro-console-producer \
...
```shell
docker compose exec kafka-1 kafka-consumer-groups --bootstrap-server kafka-1:19092 --group test-group --describe
...
```shell
docker compose exec kafka-dr-1 bash -c '\
echo "\
...
```shell
    docker compose exec kafka-dr-1 \
    kafka-cluster-links --bootstrap-server kafka-dr-1:29082 \
...
```shell
    ...
    --bootstrap-server kafka-dr-1:29082        
``` 
...
```shell
    docker compose exec kafka-dr-1 kafka-cluster-links --bootstrap-server kafka-dr-1:29082 --link main-to-disaster-cl --list
 ````
...
```shell
docker compose exec kafka-dr-1 kafka-consumer-groups --bootstrap-server kafka-dr-1:29082 --group test-group --describe
...
```shell
    docker compose exec disasterSchemaregistry \
        kafka-avro-console-consumer --bootstrap-server kafka-dr-1:29082 \
...
```shell
docker compose stop kafka-1 mainZookeeper kafka-dr-1 mainControlCenter
```
...
```shell
    docker compose exec kafka-dr-1 \
        kafka-mirrors --bootstrap-server kafka-dr-1:29082 \
...
```shell
docker compose start kafka-1 mainZookeeper kafka-dr-1 mainControlCenter
```
...
docker compose exec kafka-dr-1 bash -c '\
    schema-exporter --pause --name main-to-disaster-sl --schema.registry.url http://kafka-dr-1:8085'
# delete it
docker compose exec kafka-dr-1 bash -c '\
    schema-exporter --delete --name main-to-disaster-sl --schema.registry.url http://kafka-dr-1:8085'
```
...
```shell
# delete topic
docker compose exec kafka-1 kafka-topics --bootstrap-server kafka-1:19092 --topic product --delete
# delete the subject
...
docker compose exec disasterSchemaregistry bash -c '\
  echo "schema.registry.url=http://kafka-dr-1:8085" > /home/appuser/config.txt'
```
...
echo "\
bootstrap.servers=kafka-dr-1:29082
consumer.offset.sync.enable=true 
consumer.offset.group.filters="{\"groupFilters\": [{\"name\": \"*\",\"patternType\": \"LITERAL\",\"filterType\": \"INCLUDE\"}]}"
" > /home/appuser/cl.properties'
```
...
```shell
    ...
    kafka-cluster-links --bootstrap-server kafka-1:19092 \
...
```shell
    docker compose exec kafka-1 kafka-cluster-links --bootstrap-server kafka-1:19092 --link disaster-to-main-cl --list
 ````
...
```shell
    docker compose exec kafka-1 \
        kafka-mirrors --bootstrap-server kafka-1:19092 \
...
```shell
# delete topic
docker-compose exec kafka-dr-1 kafka-topics --bootstrap-server kafka-dr-1:29082 --topic product --delete
# delete the subject
